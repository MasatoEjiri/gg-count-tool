...
st.markdown("""### 使用方法
1. 画像を左にアップロードしてください。
2. サイドバーの「1. 輝点検出方法の選択」で解析手法を選び、各パラメータを調整してください。
...
if st.session_state.pil_image_original_full_res is not None:
    ...
    if detection_method == "明るさで検出（従来法）":
        ...
        val_min = st.sidebar.slider(
            ...
            max_value=255, 
            value=60, # ★★★ デフォルト値を60に ★★★
            help="色の明るさ。値を上げると、暗い色のノイズを除去できます。"
        )
    ...
    st.sidebar.caption("""
    オープニング処理で、小さなノイズ除去や輝点同士の分離を行います。
    ...
    if detection_method == "明るさで検出（従来法）":
        img_gray = cv2.cvtColor(np_rgb_full_uint8, cv2.COLOR_RGB2GRAY)
