...
def display_count_in_sidebar(placeholder, count_value):
    ...
    with placeholder.container(): # コンテナを使ってクリアと書き込み
...
if "binary_threshold_value" not in st.session_state: 
    st.session_state.binary_threshold_value = 58 # デフォルト58
# 以下のキーも、もしコールバックやウィジェットで使うなら初期化が必要ですが、
# ユーザー提供のスクリプトでは二値化以外はキーを多用していなかったので、
# ウィジェットのvalue引数でデフォルトを指定する形に戻します。
# ただし、二値化の閾値はコールバックで同期しているのでセッションステートを使います。
...
# --- サイドバーの定義開始 ---
# ★★★ 解析結果をサイドバーの他のどの要素よりも先に表示 ★★★
display_count_in_sidebar(result_placeholder_sidebar, st.session_state.counted_spots_value) 

st.sidebar.header("解析パラメータ設定")
UPLOAD_ICON = "📤" 
# uploaded_file_widget という名前にしていましたが、ユーザー提供版は uploaded_file でした
...
threshold_value = st.session_state.binary_threshold_value # 処理で使う値
...
morph_kernel_shape_options = {"楕円":cv2.MORPH_ELLIPSE,"矩形":cv2.MORPH_RECT,"十字":cv2.MORPH_CROSS} # ユーザー版では _display がなかった
selected_shape_name = st.sidebar.selectbox("カーネル形状",options=list(morph_kernel_shape_options.keys()),index=0) # keyなし、valueもなし(indexでデフォルト指定)
...
kernel_size_morph = st.sidebar.select_slider('カーネルサイズ',options=kernel_options_morph,value=3) # keyなし、valueでデフォルト指定
st.sidebar.caption("""- **大きくすると:** 効果強、輝点も影響あり。\n- **小さくすると:** 効果弱。""") 

st.sidebar.subheader("3. 輝点フィルタリング (面積)") 
min_area = st.sidebar.number_input('最小面積',min_value=1,max_value=10000,value=1,step=1) # ★★★ デフォルトを1に ★★★ keyなし
st.sidebar.caption("""- **大きくすると:** 小さな輝点を除外。\n- **小さくすると:** ノイズを拾う可能性。(画像リサイズ時注意)""") 
max_area = st.sidebar.number_input('最大面積',min_value=1,max_value=100000,value=1000,step=1) # keyなし
...
if st.session_state.pil_image_to_process is not None:
    ...
    ret_thresh, binary_img_processed = cv2.threshold(blurred_img,threshold_value,255,cv2.THRESH_BINARY) # threshold_value を使用
    ...
    if binary_img_for_morph_processed is not None:
        kernel_morph_obj=cv2.getStructuringElement(morph_kernel_shape,(kernel_size_morph,kernel_size_morph)) # morph_kernel_shape, kernel_size_morph を使用
    ...
    if binary_img_for_contours_processed is not None:
        ...
        if 'contours' in locals() and contours: 
            for contour in contours:
                ...
                if min_area <= area <= max_area: # min_area, max_area を使用
    ...
    st.markdown("---")
    # ★★★ 形態学的処理の表示をエキスパンダーに戻す (ユーザーの以前の要望) ★★★
    ...
    if 'contours' in locals() and contours and binary_img_for_contours_processed is not None and current_counted_spots > 0 :
         st.image(display_final_marked_image_rgb,caption=f'検出輝点(青い輪郭,面積:{min_area}-{max_area})',use_container_width=True)
    ...
    # display_count_in_sidebar の呼び出しは、ifブロックの外の初期表示と、
    # ifブロック内の画像処理完了後にあるので、ここでの重複呼び出しは削除しても良い
    # ただし、画像がクリアされた場合に "---" に戻すためにはここにも必要。
    # 現状は、画像がない場合、st.session_state.pil_image_to_process が None になり、
    # このブロックに入らず、ページ上部の display_count_in_sidebar が初期値を表示する。
    # 画像選択が解除された場合に明示的にリセットするならここにも書く。今回はシンプルに。
