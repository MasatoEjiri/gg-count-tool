...
st.markdown("""### 使用方法
1. 画像を左にアップロードしてください。
2. サイドバーの「1. 輝点検出方法の選択」で、明るさで検出するか、色で検出するかを選びます。
...
if st.session_state.pil_image_original_full_res is not None:
    ...
    if detection_method == "明るさで検出（従来法）":
        ...
        # OpenCVのHは0-179。赤色は0付近と179付近にまたがるため、2つの範囲が必要
        st.sidebar.write("色相 (Hue) の範囲 (赤色を検出する場合)")
        hue_lower1 = st.sidebar.slider("下限1 (H)", 0, 179, 0)
        hue_upper1 = st.sidebar.slider("上限1 (H)", 0, 179, 10)
        hue_lower2 = st.sidebar.slider("下限2 (H)", 0, 179, 160)
        ...
        st.sidebar.caption("彩度や明度の下限を上げることで、くすんだ色のノイズを除去できます。")
    ...
    if detection_method == "明るさで検出（従来法）":
        ...
        img_hsv = cv2.cvtColor(np_rgb_full_uint8, cv2.COLOR_RGB2HSV)
        # 赤色用の範囲を2つ設定
        lower_red1 = np.array([hue_lower1, sat_min, val_min])
        upper_red1 = np.array([hue_upper1, 255, 255])
        lower_red2 = np.array([hue_lower2, sat_min, val_min])
        upper_red2 = np.array([hue_upper2, 255, 255])
        mask1 = cv2.inRange(img_hsv, lower_red1, upper_red1)
        mask2 = cv2.inRange(img_hsv, lower_red2, upper_red2)
        binary_img = mask1 + mask2 # 2つのマスクを結合
    ...
    with st.expander("▼ 中間処理の画像を見る"):
        ...
        if binary_img is not None: 
            st.image(binary_img,caption=f'適用された二値化画像（手法: {detection_method}）')
