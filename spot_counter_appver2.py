...
def display_count_in_sidebar(placeholder, count_value):
    ...
    # placeholder.container() を使ってクリア推奨でしたが、st.empty()は直接上書きでOK
...
if "morph_shape_sb_key" not in st.session_state:
    morph_kernel_shape_options_init_for_ss = {"楕円":cv2.MORPH_ELLIPSE,"矩形":cv2.MORPH_RECT,"十字":cv2.MORPH_CROSS}
    st.session_state.morph_shape_sb_key = list(morph_kernel_shape_options_init_for_ss.keys())[0] 
...
# threshold_value_sb = st.session_state.binary_threshold_value # 処理時に直接セッションステート参照
...
selected_shape_name_sb_val = st.sidebar.selectbox("カーネル形状",options=list(morph_kernel_shape_options_display.keys()), key="morph_shape_sb_key") 
# morph_kernel_shape_sb = morph_kernel_shape_options_display[selected_shape_name_sb_val] # 処理時に直接セッションステート参照
st.sidebar.caption("輝点の形状に合わせて。") 
kernel_options_morph = [1,3,5,7,9]
kernel_size_morph_sb_val =st.sidebar.select_slider('カーネルサイズ',options=kernel_options_morph, key="morph_size_sb_key") # 処理時に直接セッションステート参照
st.sidebar.caption("""- **大きくすると:** 効果強、輝点も影響あり。\n- **小さくすると:** 効果弱。""") 
st.sidebar.subheader("3. 輝点フィルタリング (面積)") 
min_area_sb_val = st.sidebar.number_input('最小面積',min_value=1,max_value=10000,step=1, key="min_area_sb_key") # 処理時に直接セッションステート参照
st.sidebar.caption("""- **大きくすると:** 小さな輝点を除外。\n- **小さくすると:** ノイズを拾う可能性。(画像リサイズ時注意)""") 
max_area_sb_val = st.sidebar.number_input('最大面積',min_value=1,max_value=100000,step=1, key="max_area_sb_key") # 処理時に直接セッションステート参照
st.sidebar.caption("""- **大きくすると:** 大きな塊もカウント。\n- **小さくすると:** 大きな塊を除外。(画像リサイズ時注意)""") 


# --- アプリのメインタイトルと使用方法 (メインエリア) ---
st.markdown("<h1>Gra&Green<br>輝点カウントツール</h1>", unsafe_allow_html=True)
# ★★★ メインページ上部の固定結果表示とスペーサーを削除 ★★★
# result_placeholder_main = st.empty() 
# FIXED_RESULT_BOX_HEIGHT = 100
# def display_count_fixed_on_main_page(...): ...
...
if st.session_state.pil_image_to_process is not None:
    ...
    # display_count_in_sidebar(result_placeholder_sidebar, st.session_state.counted_spots_value) # 初期表示はスクリプト上部で行うので重複削除
