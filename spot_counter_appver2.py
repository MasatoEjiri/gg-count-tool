...
if st.session_state.pil_image_original is not None:
    ...
    with col_options:
        with st.container(border=True):
            st.subheader("枠の色", divider="rainbow") # ★★★ タイトルを変更 ★★★
    ...
    if detection_method == "明るさで検出":
        ...
        binary_img = cv2.threshold(blurred_img,threshold_value_to_use,255,cv2.THRESH_BINARY)[1]
    else: # 色で検出
    ...
    with col1_res:
        st.subheader("元の画像 (トリミング後)"); st.image(original_img_to_display_np_uint8, use_container_width=True)
    with col2_res:
