...
# セッションステート初期化
default_ss = {'counted_spots_value':"---","binary_threshold_value":15,"threshold_slider_for_binary":15,"threshold_number_for_binary":15,'pil_image_original_full_res':None, 'image_source_caption':"アップロードされた画像",'contour_color_name':"緑"}
...
if uploaded_file_widget is not None:
    ...
    except Exception as e: st.sidebar.error(f"アップロード画像の読み込みに失敗: {e}"); st.session_state.pil_image_original_full_res=None; st.session_state.counted_spots_value="読込エラー"; st.stop()
else: 
    if st.session_state.pil_image_original_full_res is not None: st.session_state.pil_image_original_full_res=None; st.session_state.counted_spots_value="---" 
...
if st.session_state.pil_image_original_full_res is not None:
    ...
    # ★★★ 輝点マーキング色の選択UI (スタイリッシュ版) ★★★
    st.sidebar.subheader("4. 表示設定")
    CONTOUR_COLORS = {"緑":"#28a745","青":"#007bff","赤":"#dc3545","黄":"#ffc107","ｼｱﾝ":"#17a2b8","ﾋﾟﾝｸ":"#e83e8c"}
    st.sidebar.radio(
        "輝点マーキング色を選択", options=list(CONTOUR_COLORS.keys()),
        key="contour_color_name", horizontal=True
    )
    selected_name = st.session_state.contour_color_name
    selected_hex = CONTOUR_COLORS[selected_name]
    st.sidebar.markdown(f"""
        <div style="padding-top: 5px;">
            <span style="font-size: 0.9em;">選択中の色: <b>{selected_name}</b></span>
            <div style="width: 100%; height: 25px; background-color: {selected_hex}; border: 1px solid rgba(0,0,0,0.2); border-radius: 5px; margin-top: 5px; box-shadow: inset 0 1px 2px rgba(0,0,0,0.1);"></div>
        </div>
