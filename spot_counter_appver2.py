...
if st.session_state.pil_image_original is not None:
    ...
    # ★★★ ラジオボタンの戻り値を直接使って、UIと内部状態を確実に同期 ★★★
    detection_method = st.sidebar.radio(
        "検出方法を選択",
        options=detection_options,
        index=detection_options.index(st.session_state.detection_method), # デフォルト表示を設定
        key="detection_method", # セッションステートとも連携
        horizontal=True,
    )
    st.sidebar.markdown("---")
    ...
    if detection_method == "明るさで検出（従来法）":
        ...
        threshold_value_to_use = st.session_state.binary_threshold
    else: # 色で検出（新機能）
        ...
        val_min = st.session_state.brightness
