...
if st.session_state.pil_image_to_process is not None:
    ...
    pil_for_canvas_bg = None
    np_for_canvas_bg_display = None
    ...
    canvas_result = st_canvas(
        ...
        background_image=np_for_canvas_bg_display if np_for_canvas_bg_display is not None else None, # ★★★ NumPy配列を試す ★★★
        update_streamlit=True, height=pil_for_canvas_bg.height, width=pil_for_canvas_bg.width,
        drawing_mode="rect", key="roi_canvas_main_v4" # キーを更新
    )
        ...
        if canvas_result.json_data["objects"][-1]["type"] == "rect":
            ...
            if w_cvs > 0 and h_cvs > 0:
                ...
                if (x2-x1 > 0) and (y2-y1 > 0):
                    ...
                    with st.expander("選択されたROI（処理対象のグレースケール）", expanded=True): # 表示をexpanderに
