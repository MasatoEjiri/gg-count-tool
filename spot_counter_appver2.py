...
if uploaded_file_widget is not None:
    try:
        ...
        st.session_state.image_source_caption = f"アップロード: {uploaded_file_widget.name} (元サイズ: {pil_img.width}x{pil_img.height}px)"
...
if st.session_state.pil_image_to_process is not None:
    # --- サイドバーのパラメータ設定UI ---
    # ★★★ コントラスト強調のセクションを削除 ★★★
    st.sidebar.subheader("1. 二値化") # 番号を1に戻す
    ...
    st.sidebar.subheader("2. 形態学的処理 (オープニング)") # 番号を2に戻す
    ...
    st.sidebar.subheader("3. 輝点フィルタリング (面積)") # 番号を3に戻す
    ...
    st.sidebar.subheader("4. 表示設定") # 番号を4に戻す
    ...
    # ★★★ コントラスト強調処理を削除 ★★★
    # img_for_binarization = img_gray.copy() 
    
    blurred_img = cv2.GaussianBlur(img_gray, (kernel_size_blur,kernel_size_blur),0) # img_grayを直接使用
    ...
    with st.expander("▼ 2. 形態学的処理後を見る", expanded=False): # 番号を更新
    ...
    st.markdown("---") 
    st.subheader("3. 輝点検出とマーキング") # 番号を更新
    display_final_marked_image_rgb = cv2.cvtColor(output_image_contours_display, cv2.COLOR_BGR2RGB)
    if 'contours' in locals() and contours and binary_img_for_contours_processed is not None and current_counted_spots > 0 :
         st.image(display_final_marked_image_rgb,caption=f'検出輝点(選択色,面積:{min_area_to_use}-{max_area_to_use})')
