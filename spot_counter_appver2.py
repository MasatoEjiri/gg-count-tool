...
# セッションステート初期化
default_ss = {'counted_spots_value':"---", "binary_threshold_value":58, 
              "threshold_slider_for_binary":58, "threshold_number_for_binary":58,
              "morph_shape_sb_key":"楕円", "morph_size_sb_key":3,
              "min_area_sb_key_v3":1, "max_area_sb_key_v3":1000,
              'pil_image_to_process':None, 'image_source_caption':"アップロードされた画像",
              'roi_coords':None, 'last_uploaded_filename_for_roi_reset':None}
...
if uploaded_file_widget is not None:
    ...
    try:
        ...
        pil_img_original_full_res = Image.open(io.BytesIO(uploaded_file_bytes)) # フル解像度で保持
        st.session_state.pil_image_to_process = pil_img_original_full_res # セッションにもフル解像度を
        st.session_state.image_source_caption = f"アップロード: {uploaded_file_widget.name}"
    except Exception as e:
        st.sidebar.error(f"アップロード画像の読み込み失敗: {e}"); st.session_state.pil_image_to_process=None; st.session_state.counted_spots_value="読込エラー"; st.stop()
else: 
...
if st.session_state.pil_image_to_process is not None:
    ...
    # --- st_canvas 用の背景画像準備 (縮小) ---
    pil_for_canvas_bg = pil_image_rgb_full_res.copy()
    CANVAS_MAX_DIM = 800 # キャンバス表示の最大辺長
    ...
    # デバッグ表示 (縮小されたキャンバス背景候補)
    with st.expander("キャンバス背景候補の確認（デバッグ用、縮小版）", expanded=False):
        st.image(pil_for_canvas_bg, caption=f"キャンバス背景 ({pil_for_canvas_bg.width}x{pil_for_canvas_bg.height})", use_container_width=True)
    ...
    img_for_marking_color_np = np_array_rgb_uint8_full_res.copy() # マーキングはフル解像度カラー画像に
    ...
    if canvas_result.json_data is not None and canvas_result.json_data.get("objects", []):
        if canvas_result.json_data["objects"][-1]["type"] == "rect":
            ...
            # キャンバス上の座標を取得し、元のフル解像度画像上の座標にスケーリング
            x_cvs, y_cvs, w_cvs, h_cvs = int(rect["left"]), int(rect["top"]), int(rect["width"]), int(rect["height"])
            if w_cvs > 0 and h_cvs > 0:
                x_orig = int(x_cvs * scale_x)
                y_orig = int(y_cvs * scale_y)
                w_orig = int(w_cvs * scale_x)
                h_orig = int(h_cvs * scale_y)
                ...
                if (x2-x1 > 0) and (y2-y1 > 0):
                    st.session_state.roi_coords = (x1,y1,x2-x1,y2-y1) # フル解像度でのROI座標
                    img_to_process_gray = img_gray_full_res[y1:y2, x1:x2].copy()
                    img_for_marking_color_np = np_array_rgb_uint8_full_res[y1:y2, x1:x2].copy() # マーキングもROI部分に
                    ...
                    st.image(img_to_process_gray, caption=f"ROI: x={x1},y={y1},w={x2-x1},h={y2-y1} (フル解像度座標)", use_container_width=True)
