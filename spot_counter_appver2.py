...
if st.session_state.pil_image_original is not None:
    ...
    if st.session_state.detection_method == "明るさで検出（従来法）":
        ...
        st.session_state.binary_threshold_value = st.sidebar.slider('閾値 (スライダーで調整)',min_value=0,max_value=255,step=1,key="threshold_slider",on_change=sync_threshold_from_slider)
        ...
        st.session_state.saturation_value = st.sidebar.slider("彩度(S)の下限", min_value=0, max_value=255, key="saturation_slider", on_change=sync_saturation_from_slider, help="色の「鮮やかさ」の最低ライン。値を上げると、白やグレーに近いくすんだ色が除外されます。")
        ...
        st.session_state.brightness_value = st.sidebar.slider("明度(V)の下限", min_value=0, max_value=255, key="brightness_slider", on_change=sync_brightness_from_slider, help="色の「明るさ」の最低ライン。値を上げると、暗い部分にあるノイズが除外されます。")
    ...
    with st.expander("▼ 中間処理の画像を見る"):
        st.subheader(f"1. {st.session_state.detection_method}による二値化処理後"); st.image(binary_img,caption=f'適用された二値化画像')
