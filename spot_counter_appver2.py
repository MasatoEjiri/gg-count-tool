...
if st.session_state.pil_image_original is not None:
    ...
    if detection_method == "明るさで検出":
        ...
        # ★★★ 色相(Hue)の範囲を選択するスライダーを追加 ★★★
        st.session_state.hue_range = st.sidebar.slider(
            "色相(Hue)の範囲", 0, 179, st.session_state.hue_range,
            help="検出したい輝点の色のおおまかな範囲を指定します。例: 赤(0-20), 黄(20-35), 緑(35-85), シアン(85-100), 青(100-130), マゼンタ(130-160)")
    ...
    # --- 共通のパラメータ ---
    st.sidebar.subheader("3. 形態学的処理"); kernel_size_morph_to_use =st.sidebar.select_slider('カーネルサイズ',options=[1,3,5,7,9],value=1, help="ノイズ除去や輝点分離の効果の強さを調整します。"); erosion_iterations = 1 
    ...
    if detection_method == "明るさで検出":
        ...
        # もし赤色のようにHueが0と179をまたぐ場合は、別途UIの工夫が必要（例：範囲を2つ設定）
