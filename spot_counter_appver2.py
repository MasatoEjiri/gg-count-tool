...
# ★★★ セッションステートの内容を表示（デバッグ用）★★★
with st.sidebar.expander("セッションステート確認（デバッグ用）"):
    st.json(st.session_state) # オブジェクトを展開可能な形で表示
...
if st.session_state.pil_image_to_process is not None:
    ...
    ret_thresh, binary_img_processed = cv2.threshold(blurred_img,st.session_state.binary_threshold_value,255,cv2.THRESH_BINARY) # 修正: 直接セッションステート参照
    ...
    if binary_img_for_morph_processed is not None:
        kernel_morph_obj=cv2.getStructuringElement(morph_kernel_shape_options_display[st.session_state.morph_shape_sb_key],(st.session_state.morph_size_sb_key,st.session_state.morph_size_sb_key)) # 修正
    ...
    if binary_img_for_contours_processed is not None:
        ...
        if 'contours' in locals() and contours: 
            for contour in contours:
                ...
                if st.session_state.min_area_sb_key <= area <= st.session_state.max_area_sb_key: # 修正
    ...
    if binary_img_processed is not None: st.image(binary_img_processed,caption=f'閾値:{st.session_state.binary_threshold_value}',use_container_width=True) # 修正
    ...
    if 'contours' in locals() and contours and binary_img_for_contours_processed is not None and current_counted_spots > 0 :
         st.image(display_final_marked_image_rgb,caption=f'検出輝点(青い輪郭,面積:{st.session_state.min_area_sb_key}-{st.session_state.max_area_sb_key})',use_container_width=True) # 修正
