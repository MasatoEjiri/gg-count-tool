...
if st.session_state.pil_image_original is not None:
    ...
    if detection_method == "明るさで検出（従来法）":
        ...
        sat_min = st.sidebar.slider("彩度(Saturation)の下限", 0, 255, 100, help="色の鮮やかさ。値を上げると、くすんだ色のノイズを除去できます。")
        val_min = st.sidebar.slider("明度(Value)の下限", 0, 255, 100, help="色の明るさ。値を上げると、暗いノイズを除去できます。")
    ...
    with st.expander("▼ 中間処理の画像を見る"):
        st.subheader(f"1. 二値化処理後 ({detection_method})")
