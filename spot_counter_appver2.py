...
st.markdown("""### 使用方法
1. 画像を左にアップロードしてください。
2. メイン画面に表示された画像の上で、四角い枠をドラッグ＆リサイズして解析したいエリアを選択します。枠の色はサイドバーの「1. 解析エリア設定」で変更できます。
3. 左サイドバーの「2. 二値化」以降の各パラメータを調整し、「元の画像（トリミング後）」と「輝点検出とマーキング」を比較しながら最適な設定を見つけてください。
""")
st.markdown("---") 
