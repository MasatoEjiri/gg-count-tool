...
if uploaded_file_widget is not None:
    try:
        ...
        st.session_state.pil_image_original_full_res = pil_img_original # フル解像度を保持
        original_dims = f"(元サイズ: {pil_img_original.width}x{pil_img_original.height}px)"
        
        # 表示・処理用の画像をリサイズ (セッションステートにはフル解像度を保持)
        pil_img_for_processing = resize_image(pil_img_original.copy()) 
        resized_dims = f"(処理サイズ: {pil_img_for_processing.width}x{pil_img_for_processing.height}px)"
        
        # st.session_state.pil_image_to_process = pil_img_for_processing # pil_image_to_process はもう使わない
        st.session_state.image_source_caption = f"アップロード: {uploaded_file_widget.name} {original_dims} {resized_dims if pil_img_original.size != pil_img_for_processing.size else ''}"
...
if st.session_state.pil_image_original_full_res is not None:
    ...
    st.sidebar.slider('閾値 (スライダーで調整)',
                      min_value=15, # ★★★ 下限値を15に変更 ★★★
    ...
    st.sidebar.number_input('閾値 (直接入力)',
                             min_value=15, # ★★★ 下限値を15に変更 ★★★
                             max_value=255,step=1,
                             value=st.session_state.binary_threshold_value, 
    ...
    contour_color_bgr = tuple(int(contour_color_hex.lstrip('#')[i:i+2], 16) for i in (4, 2, 0))
    ...
    def create_display_version_pil(pil_image, target_width): # Pillow入力、Pillow出力
        if pil_image is None: return None
        img_copy = pil_image.copy()
        if img_copy.width > target_width : # 幅がtarget_widthを超える場合のみリサイズ
    ...
    if marked_img_for_display is not None:
        if current_counted_spots > 0 :
             ...
             st.image(marked_img_for_display,caption='輝点見つからず')
