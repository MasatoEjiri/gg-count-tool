...
if st.session_state.pil_image_to_process is not None:
    ...
    with st.expander("▼ 2. 形態学的処理後を見る", expanded=False): 
        if opened_img_processed is not None: 
            # ★★★ use_container_width=True を削除 ★★★
            st.image(opened_img_processed,caption=f'カーネル:{st.session_state.morph_shape_sb_key} {st.session_state.morph_size_sb_key}x{st.session_state.morph_size_sb_key}')
    ...
    if 'contours' in locals() and contours and binary_img_for_contours_processed is not None and current_counted_spots > 0 :
         # ★★★ use_container_width=True を削除 ★★★
         st.image(display_final_marked_image_rgb,caption=f'検出輝点(青い輪郭,面積:{st.session_state.min_area_sb_key_v3}-{st.session_state.max_area_sb_key_v3})')
