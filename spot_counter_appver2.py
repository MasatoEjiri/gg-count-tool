...
st.markdown("""### 使用方法
1. 画像を左にアップロードしてください。
2. サイドバーの「1. 輝点検出方法」で解析手法を選び、続く各パラメータを調整してください。
3. 「元の画像」と「輝点検出とマーキング」を比較しながら最適な設定を見つけます。
""")
st.markdown("---") 
...
if st.session_state.pil_image_to_process is not None:
    ...
    detection_method = st.sidebar.radio("検出方法を選択", ("色で検出", "明るさで検出"), key="detection_method", horizontal=True)
    st.sidebar.markdown("---")
    
    if detection_method == "明るさで検出":
        st.sidebar.subheader("2. 二値化")
        st.session_state.binary_threshold = st.sidebar.slider(
            '閾値',
            min_value=0,max_value=255,
            value=st.session_state.binary_threshold, 
        ...
        st.session_state.saturation = st.sidebar.slider(
            "彩度(S)の下限", 
            min_value=0, max_value=255, 
            value=st.session_state.saturation, 
            help="色の「鮮やかさ」の最低ライン。値を上げると、白やグレーに近いくすんだ色が除外されます。"
        )
        st.session_state.brightness = st.sidebar.slider(
            "明度(V)の下限", 
            min_value=0, max_value=255, 
    ...
    kernel_size_morph_to_use =st.sidebar.select_slider(
        ...
        options=[1,3,5,7,9],
        value=1, 
        help="ノイズ除去や輝点分離の効果の強さを調整します。値を大きくすると効果は強まりますが、輝点自体が消える可能性もあります。"
    )
    ...
    except Exception as e: st.error(f"画像の基本変換に失敗: {e}"); st.stop() 
    ...
    with col1_res:
        st.subheader("元の画像"); st.image(original_img_to_display_np_uint8, use_container_width=True)
    with col2_res:
