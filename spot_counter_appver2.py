...
if 'contour_color_name' not in st.session_state: st.session_state.contour_color_name = "緑" # 色選択のデフォルト
...
5. 「3. 輝点検出とマーキング」のセクションで、マーキングの色を変更できます。
""")
st.markdown("---") 
...
if st.session_state.pil_image_to_process is not None:
    ...
    st.subheader("元の画像")
    if original_img_to_display_np_uint8 is not None: st.image(original_img_to_display_np_uint8, caption=st.session_state.image_source_caption, width=600)
    st.markdown("---")
    st.subheader("1. 二値化処理後")
    if binary_img_processed is not None: st.image(binary_img_processed,caption=f'閾値:{threshold_value_to_use}', width=600)
    else: st.info("二値化未実施/失敗")
    st.markdown("---")
    with st.expander("▼ 2. 形態学的処理後を見る", expanded=False): 
        if opened_img_processed is not None: st.image(opened_img_processed,caption=f'カーネル: 楕円 {kernel_size_morph_to_use}x{kernel_size_morph_to_use}', width=600)
    ...
    # ★★★ 輪郭色選択UIをメインエリアに移動 ★★★
    with st.expander("マーキング色の変更"):
        CONTOUR_COLORS = {"緑":"#28a745","青":"#007bff","赤":"#dc3545","黄":"#ffc107","ｼｱﾝ":"#17a2b8","ﾋﾟﾝｸ":"#e83e8c"}
        def format_color_option(color_name):
            emojis = {"緑": "🟢", "青": "🔵", "赤": "🔴", "黄": "🟡", "シアン": "💧", "ピンク": "🌸"}
            return f"{emojis.get(color_name, '🎨')} {color_name}"
        st.radio("輝点マーキング色を選択", options=list(CONTOUR_COLORS.keys()), format_func=format_color_option, key="contour_color_name", horizontal=True)
        selected_name = st.session_state.contour_color_name
        selected_hex = CONTOUR_COLORS[selected_name]
        st.markdown(f"""<div style="text-align: left; margin-top: 5px;"><span style="font-size: 0.9em;">選択中の色: <b>{selected_name}</b></span><div style="width: 100%; height: 25px; background-color: {selected_hex}; border: 1px solid rgba(0,0,0,0.2); border-radius: 5px; margin-top: 5px;"></div></div>""", unsafe_allow_html=True)
