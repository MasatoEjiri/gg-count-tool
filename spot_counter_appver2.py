...
if uploaded_file_widget is not None:
    # 新しいファイルがアップロードされたら、以前の処理済み画像情報をクリアする（任意）
    # st.session_state.pil_image_to_process = None 
    try:
        uploaded_file_bytes = uploaded_file_widget.getvalue()
        pil_img = Image.open(io.BytesIO(uploaded_file_bytes))
        st.session_state.pil_image_to_process = pil_img # 処理対象の画像をセッションステートに保存
    ...
    # ファイルアップローダーがクリアされた場合（ユーザーが×ボタンを押した場合など）
...
if st.session_state.pil_image_to_process is not None:
    ...
    # st.session_state.counted_spots_value = "---" # 既に初期化済み、または画像クリア時に設定される
