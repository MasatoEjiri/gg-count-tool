...
def display_count_fixed_top_left(placeholder, count_value): # ★★★ 関数定義 ★★★
...
# 固定表示ボックスのためのスペーサー
SPACER_HEIGHT_FOR_FIXED_BOX = 20 + FIXED_RESULT_BOX_APPROX_HEIGHT + 20 
...
# --- メインページ上部の結果表示の初期呼び出し (タイトルより前、関数の定義後) ---
...
kernel_options_morph = [1,3,5,7,9]; kernel_size_morph_sb =st.sidebar.select_slider('カーネルサイズ',options=kernel_options_morph, value=st.session_state.morph_size_sb_key, key="morph_size_sb_key")
...
if st.session_state.pil_image_to_process is not None:
    ...
    # display_count_fixed_top_left は初期表示済みなので、ここでは呼ばなくてもよい
    # (もし画像がクリアされた場合に "---" に戻したいなら必要)
    if uploaded_file_widget is None and st.session_state.pil_image_to_process is None : # 明示的にクリアされた場合など
        st.session_state.counted_spots_value = "---"
        display_count_fixed_top_left(result_placeholder_main, st.session_state.counted_spots_value)
